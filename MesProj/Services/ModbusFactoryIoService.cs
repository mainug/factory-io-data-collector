...
namespace MesProj.Services
{
    public sealed class ModbusFactoryIoService : IFactoryIoService
    {
        ...
        private readonly BlueLidSorterController _blueLidSorter = new BlueLidSorterController();
        ...
        public async Task ConnectAsync(CommunicationOptions options, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                _blueLidSorter.Reset();
                await _blueLidSorter.StopOutputsAsync(
                    SetSorterOutputAsync,
        ...
        public async Task WriteCoilAsync(int address, bool value, CancellationToken cancellationToken)
        {
            ...
            _blueLidSorter.ValidateOutputCommand(
                address,
                value,
                key =>
                {
                    lock (_syncRoot)
                    {
                        return _statuses[key].CommandState;
                    }
                });
        ...
        public async Task SetBlueLidAutoSortingEnabledAsync(bool enabled, CancellationToken cancellationToken)
        {
            ...
            await _blueLidSorter.SetEnabledAsync(
                enabled,
                SetSorterOutputAsync,
                cancellationToken).ConfigureAwait(false);
            RaiseStatus();
                ...
                try
                {
                    foreach (var definition in _definitions.Where(x => x.FeedbackInputAddress >= 0))
                    {
                        ...
                        if (risingEdge || fallingEdge)
                        {
                            ...
                            lock (_syncRoot)
                            {
                                resetRecoveryPending = _resetRecoveryPending;
                            }
                            await _blueLidSorter.HandleSensorEdgeAsync(
                                definition,
                                risingEdge,
                                fallingEdge,
                    ...
                    await _blueLidSorter.CheckTimersAsync(
                        SetSorterOutputAsync,
                        RaiseSorterAlarm,
                        cancellationToken).ConfigureAwait(false);
